...
- **Your token and pool.** Supply is fixed by the factory, the pool position is locked in the pons locker forever. Nobody, including robindrop, can pull liquidity.
- **"Keep it" mode.** Fees go to your wallet. robindrop never appears in the flow.

## What is custodial

**The fee stream in managed modes.** When you pick holder drops, a split, or burn, the `feeWallet` is a robindrop vault:
...
This is a deliberate trade: a server-held key is what lets a bot pay 200 holders every round without anyone signing 200 prompts. If that trade is not for you, pick **Keep it**: it costs you nothing and robindrop stays out of the loop.
...
- A robindrop server outage pauses distributions; fees keep accruing in the locker and vault, nothing is lost.
